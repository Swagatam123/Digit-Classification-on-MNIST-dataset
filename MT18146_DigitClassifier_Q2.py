...
for i in range(0,5):
    ...
    test_fold_dataset,test_fold_label,train_fold_dataset,train_fold_label=calculate5folding(i,total_fold,total_fold_label,part)
    ...
    for j in range(0,len(train_fold_label)):
        if train_fold_label[j]==1:
            train_1_dataset.append(train_fold_dataset[j])
        if train_fold_label[j]==8:
            train_8_dataset.append(train_fold_dataset[j])
    actual_1=0
    actual_8=0
    for j in range(0,len(train_label_dataset)):
        if train_label_dataset[j]==1:
            actual_1+=1
        elif train_label_dataset[j]==8:
            actual_8+=1
    ...
    predicted_8=0
    posterior_1_dataset=[]
    binarized_test_dataset=binarize(train_dataset)
    for i in range(0,len(binarized_test_dataset)):
        feature = binarized_test_dataset[i]
        ...
        posterior_1_dataset.append(posterior_probability_1)
        label=0
        if posterior_probability_1 > posterior_probability_8:
            label=1
        else:
            label=8
        if label==train_label_dataset[i]:
            true+=1
            if label==1:
                confusion_matrix[0][0]+=1
            if label==8:
                confusion_matrix[1][1]+=1
        elif label==1 and train_label_dataset[i]==8:
            confusion_matrix[0][1]+=1
        elif label==8 and train_label_dataset[i]==1:
            confusion_matrix[1][0]+=1
        if label==1:
          predicted_1+=1
        else:
          predicted_8+=1
    print("precision : ",true/len(train_label_dataset))
    pr_list.append(true/len(train_label_dataset))
x_points,y_points=DET_curve_points(posterior_1_dataset,binarized_test_dataset,train_label_dataset,actual_1,actual_8)
...
#print(numpy.mean(pr_list))
#print(numpy.std(pr_list))
